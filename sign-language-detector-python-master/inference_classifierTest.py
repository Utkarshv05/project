...
hand_detected = False  # Track whether a hand is detected
    ...
    if results.multi_hand_landmarks:
        ...
        if predicted_character == predicted_character_new:
            ...
            if stable_prediction_count >= stable_threshold:
                # Add the character to the buffer if it's new or hand re-enters
                if len(text_buffer) == 0 or predicted_character != text_buffer[-1]:
        ...
        # If no hand is detected, reset prediction
        if hand_detected:  # Only reset when hand was detected previously
            predicted_character = None  # Allow the same letter to be detected again
            stable_prediction_count = 0  # Reset the stability counter
            hand_detected = False  # Update the hand detection status
