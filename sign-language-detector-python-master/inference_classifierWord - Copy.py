...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if stable_prediction_count >= stable_threshold:
            exit_loop = False  # Flag to control exiting the block
    
        if len(text_buffer) == 0 or predicted_character != text_buffer[-1]:
            text_buffer += predicted_character_new
            exit_loop = True  # Set the flag to exit
    
        if not exit_loop and predicted_character == text_buffer[-1]:
            stable_threshold = 10
            exit_loop = True  # Set the flag to exit
    
        if not exit_loop and predicted_character == text_buffer[-1] and stable_prediction_count == 10:
            stable_threshold = 5
            text_buffer += predicted_character_new
            exit_loop = True  # Set the flag to exit
